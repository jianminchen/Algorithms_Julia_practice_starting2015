 ...
 * 
 * Solution notes:
 * This problem can be solved using Trie and DP. Create a trie from the given set of strings. 
 * Find the prefix neighbor of each string. Now create a graph such that each string is a node 
 * and there exist a bidirectional edge between two nodes only if they are prefix neighbors. 
 * Now find the maximum weighted independent set.
 * 
...
class Solution
{
    ...
    internal class Trie
    {
            ...
            string neighbour)
        {
            ...
            // update neighbor string - if IsWord, then it is to set as a prefix neighbor
            return Children[visiting].AddWordToTrieByOneCharATime(scanIndex + 1, charArray, word, IsInDictionary ? LastVisitedWord : neighbour);            
        }
    }
