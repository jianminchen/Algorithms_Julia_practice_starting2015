    ...
    class Solution
    {
        ...
        public class Graph
        {
            ...
            //one edge with one edge id, but use negative edge id as well to accomodate two nodes in the edge.  
            ...
            /// is a linked list with 2 nodes, where first node is edge 1 which are connected by Id 1 and Id 6, 
            /// and the second node is edge 5 which are connected by Id 2 and Id 2. 
            /// </summary>
            internal class Node
            {
            ...
            internal class Edge
            {
                ...
                /// <summary>
                /// one edge has one edge id and two nodes with two node's Ids, then the node with large node id
                /// takes the positive edge id whereas the small one uses negative edge id.         
                /// </summary>
                ...
                /// <summary>
                /// one edge has two node ids, and then one edge id and its negative value both
            ...
            /// First edge with edgeId = 1, node Id 6 connects to node Id 1, so node id 6 only 
            /// has one extra connected edge to node 3, which only has one connected edge.  
            /// Node Id = 1, which has connected edge 5(1-2), then DFS search to edge 4(5-2), then DFS
            /// search edge 2 (4-5), node 4 has only one connected edge which is the base case.  
            /// </summary>
            /// <param name="signedEdgeId"></param>
