...
{   
    ...
    class Graph
    {
        ...
        internal class Edge 
        {
            ...
            /// <param name="b"></param>
            /// <returns></returns>
            public static int Compare_problem(Edge a, Edge b)
            {
                int first  =  a.consequent * b.antecedent;
                int second =  b.consequent * a.antecedent;
               
                if (first == second) 
                {   // 1/2 vs 2/4 , a/b, choose 1/2
                    if (a.consequent > a.antecedent)
                    {
                        return a.antecedent - b.antecedent;  // smaller is to put first after sorting
                    }
                    else
                    {
                        return b.antecedent - a.antecedent; 
                    }
                }
                else
                {
            ...
            {
                bool aSmallerThan1 = QuotientSmallerThanOne(a);

                bool bSmallerthan1 = QuotientSmallerThanOne(b);

                if (aSmallerThan1 && bSmallerthan1)
                {
                    return b.consequent - a.consequent;  // keep smaller one
                }
                else if (!aSmallerThan1 && !bSmallerthan1)
                {
                    return b.antecedent - a.antecedent;  // keep bigger one
                }
                else if (aSmallerThan1)
                {
                    return 1;
                }
                else return 0; 
            ...
            public static bool QuotientSmallerThanOne(Edge a)
            {
                return a.antecedent < a.consequent; 
            }
        };
        ...
        public static void ProcesInput()
        {
            ...
            for (int i = 0; i < noEdges; i++)
            {
                ...
                if (lookup.ContainsKey(key) )
                {
                    ...
                    bool currentSmallerThan1 = consequent > antecedent;
                    bool oldSmallerthan1     = oldConsequent > oldAntecedent;                                      

                    // replace the old one 
                    if ((currentSmallerThan1 && oldSmallerthan1 && (oldConsequent - consequent) > 0) ||
                        (!currentSmallerThan1 && !oldSmallerthan1 && (antecedent - oldAntecedent) > 0) || 
                        (currentSmallerThan1 && !oldSmallerthan1))                         
                    {                        
                        lookup[key] = node;
                    }                   
                }
                ...
                else if (lookup.ContainsKey(alternativeKey))
                {
                    ...
                    if (currentWeight < oldWeight ||
                        ...
                        ((consequent < oldConsequent && consequent > antecedent) ||
                         (consequent > oldConsequent && consequent < antecedent)
                        )))
                    {
