...
namespace zeroOneGame
{
    class Program
    {
        ...
        {
            List<int> numbers = new List<int> { 1, 0, 1, 0, 1 };
        ...
        public static void ProcessInput()
        {
            ...
            for (int i = 0; i < queries; i++)
            {
                ...
                var numbersEasyToRemove = new List<int>(numbers);
        ...
        /// <returns></returns>
        public static int CountMoves(List<int> numbers, int start)
        {
                ...
                if (previous == 0 && next == 0)
                {
                   ...
                   // try to solve timeout issue
                    ...
                    if( i - 2 >=0 )
                    {
                        if(numbers[i-2] == 1)
                        {
                            numbers.RemoveRange(0, i - 2);
                            return 1 + CountMoves(numbers, 0);
                        }
                        else 
                        {
                            return 1 + CountMoves(numbers, i - 2);
